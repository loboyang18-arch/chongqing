#!/usr/bin/env python3
"""V25 方案1联合搜索：AFTER=0 固定，BEFORE × λ 网格（验证集选模）。

固定：V25_DUAL=1, V18_CTX_AFTER=0。
每个 trial 仅 val 评估；结束后对 val MAE 最优配置重训并跑测试集。

示例：
  python run_v25_ctx_lambda_search_after0.py
  python run_v25_ctx_lambda_search_after0.py --skip-test
  python run_v25_ctx_lambda_search_after0.py --before 5 6 --lambda 0.15 0.2 0.25
"""
...
def _run_trial(
    ...
    out_rel = f"v25_ctx_lambda_after0_search/{run_id}/trials/{tid}"
    ...
    log.info("RUN %s | BEFORE=%d λ=%s AFTER=0", tid, before, lam)
    t0 = time.time()
    proc = subprocess.run(
...
def _write_leaderboard(
    ...
    ordered = ok + failed
    headers = [
        "rank", "trial_id", "status", "V18_CTX_BEFORE", "V18_CTX_AFTER",
...
def _run_best_test(best: dict[str, Any], search_dir: Path, log: logging.Logger) -> Path:
    ...
    out_rel = "v25_resconv_after0_ctx_lambda_best"
    env = os.environ.copy()
    env.update({
        "V25_DUAL": "1",
        "V18_DELTA_LAMBDA": str(lam),
    ...
    log.info("Retrain best → %s (BEFORE=%d λ=%s)", out_rel, before, lam)
...
def _load_test_metrics(out_dir: Path) -> dict[str, Any]:
    p = out_dir / "metrics_eval_standard_da_result.json"
...
def main() -> None:
    ap = argparse.ArgumentParser(description="V25: AFTER=0, search BEFORE×λ on val")
    ap.add_argument(
        "--before", type=int, nargs="+", default=[4, 5, 6, 7, 8, 9],
        help="BEFORE grid (default 4..9, skip 3: arch min slots)",
    )
    ap.add_argument(
    ...
    search_dir = OUTPUT_DIR / "v25_ctx_lambda_after0_search" / run_id
    ...
    log = logging.getLogger("v25_ctx_lambda")
    combos = list(itertools.product(args.before, args.lambdas))
    log.info("Search dir: %s", search_dir)
    log.info("AFTER=0 | %d trials (BEFORE×λ = %d×%d) | epochs=%d",
             len(combos), len(args.before), len(args.lambdas), args.epochs)
    ...
    log.info("leaderboard_by_val_mae: %s", search_dir / "leaderboard_by_val_mae.csv")
    if ok_mae:
        b = ok_mae[0]
        log.info(
            "BEST val MAE: BEFORE=%s λ=%s MAE=%.4f corr=%.4f score=%s",
            b["V18_CTX_BEFORE"], b["V18_DELTA_LAMBDA"],
            b["val_mae"], b["val_profile_corr"], b.get("rank_score"),
        )
    if ok_score:
        s = ok_score[0]
        log.info(
